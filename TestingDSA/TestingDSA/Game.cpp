...
void loadRecords(Record records[], int &size, int &memCounter) {
  ...
  memCounter =
      0; // Set memCounter after the loop based on the last record processed
  if (size > 0) {
...
void saveRecords(Record records[], int size, int memCounter) {
  ...
  if (file.fail()) {
    ...
    file << "gameName,winner,players" << endl; // header
...
void merge(Record records[], int first, int mid, int last) {
  ...
  // while neither subarray is empty, copy the smaller item into the temporary
  // array
