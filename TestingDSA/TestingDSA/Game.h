...
public:
  // constructor
  Game();
  Game(string name, int minPlayers, int maxPlayers, int minTime, int maxTime,
       int year, bool isBorrowed, int returnDate);
  ...
  // returns -1(if not borrowed) or the number of days between today's date and
  // borrowDate
  int borrowedDays();
